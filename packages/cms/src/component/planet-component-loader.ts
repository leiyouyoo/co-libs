...
export class PlanetComponentLoader {
  ...
  private getPlantAppRef(name: string): Observable<PlanetApplicationRef> {
    if (globalPlanet.apps[name] && globalPlanet.apps[name].appModuleRef) {
      ...
      const applicationService: any = getApplicationService();
      const applicationLoader: any = getApplicationLoader();
      const app = applicationService.getAppByName(name);
      return applicationLoader.preload(app).pipe(
  ...
  private createInjector<TData>(appModuleRef: NgModuleRef<any>, componentRef: PlanetComponentRef<TData>): PortalInjector {
  ...
  private attachComponent<TData>(
    ...
    const componentRef: any = portalOutlet.attachComponentPortal<TData>(componentPortal);
