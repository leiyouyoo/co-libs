...
subtitle: 路由复用标签
cols: 1
module: import { ReuseTabModule } from '@co/cbc/reuse-tab';
---
...
默认 `ReuseTabModule` 并不会注册 `RouteReuseStrategy`，这是因为若默认在模块内注册会导致所有引入 `@co/cbc` 模块都会强制使用路由复用（不管是否模板是否包括 `<reuse-tab>`）。因此：
