...
import { CoThemeHttpClientConfig, CO_CONFIG, deepCopy } from '@co/core';
...
describe('theme: http.client', () => {
  ...
  function createModule(config?: CoThemeHttpClientConfig) {
    const providers: any[] = [_HttpClient];
    if (config) {
      providers.push({ provide: CO_CONFIG, useValue: { themeHttp: config } });
    }
