...
import { isShowSearchObject, CoCascaderComponentAsSource, CoCascaderFilter, CoCascaderOption, CoCascaderSearchOption } from './typings';
 ...
 */
@Injectable()
export class CoCascaderService implements OnDestroy {
  /** Activated options in each column. */
  activatedOptions: CoCascaderOption[] = [];

  /** An array to store cascader items arranged in different layers. */
  columns: CoCascaderOption[][] = [];

  /** If user has entered searching mode. */
  inSearchingMode = false;

  /** Selected options would be output to user. */
  selectedOptions: CoCascaderOption[] = [];
  ...
  private columnsSnapshot: CoCascaderOption[][] = [[]];

  /** To hold activated options before entering searching mode. */
  private activatedOptionsSnapshot: CoCascaderOption[] = [];

  private cascaderComponent!: CoCascaderComponentAsSource;

  /** Return cascader options in the first layer. */
  get nzOptions(): CoCascaderOption[] {
   ...
   */
  withComponent(cascaderComponent: CoCascaderComponentAsSource): void {
   ...
   */
  withOptions(options: CoCascaderOption[] | null): void {
   ...
   */
  setOptionActivated(option: CoCascaderOption, columnIndex: number, performSelect: boolean = false, loadingChildren: boolean = true): void {
    if (option.disabled) {
      return;
    }
  ...
  setOptionSelected(option: CoCascaderOption, index: number): void {
    const changeOn = this.cascaderComponent.nzChangeOn;
    const shouldPerformSelection = (o: CoCascaderOption, i: number): boolean => {
   ...
   * @param option
   */
  setSearchOptionSelected(option: CoCascaderSearchOption): void {
  ...
  prepareSearchOptions(searchValue: string): void {
    const results: CoCascaderOption[] = []; // Search results only have one layer.
    const path: CoCascaderOption[] = [];
    const defaultFilter: CoCascaderFilter = (i, p) => {
      return p.some(o => {
    ...
    const loopChild = (node: CoCascaderOption, forceDisabled = false) => {
      path.push(node);
      const cPath = Array.from(path);
      if (filter(searchValue, cPath)) {
        const disabled = forceDisabled || node.disabled;
        const option: CoCascaderSearchOption = {
          disabled,
          isLeaf: true,
          path: cPath,
    ...
    const loopParent = (node: CoCascaderOption, forceDisabled = false) => {
      const disabled = forceDisabled || node.disabled;
      path.push(node);
  ...
  getOptionLabel(o: CoCascaderOption): string {
    return o[this.cascaderComponent.nzLabelProperty || 'label'] as string;
  }

  getOptionValue(o: CoCascaderOption): NzSafeAny {
   ...
   */
  private setColumnData(options: CoCascaderOption[], columnIndex: number, parent: CoCascaderOption): void {
   ...
   */
  loadChildren(option: CoCascaderOption | NzSafeAny, columnIndex: number, success?: VoidFunction, failure?: VoidFunction): void {
   ...
   */
  private findOptionWithValue(columnIndex: number, value: CoCascaderOption | NzSafeAny): CoCascaderOption | null {
