...
| `[nzOptionRender]` | render template of cascader options | `TemplateRef<{ $implicit: CoCascaderOption, index: number }>` | |
...
| `[nzShowSearch]` | Whether support search. Cannot be used with `[nzLoadData]` at the same time | `boolean\|CoShowSearchOptions` | `false` |
...
| `(nzSelectionChange)` | Emit on values change | `EventEmitter<CoCascaderOption[]>` | - |

When `nzShowSearch` is an object it should implements `CoShowSearchOptions`：

| Params | Explanation | Type | Default |
| --- | --- | --- | --- |
| `filter` | Optional. Be aware that all non-leaf CascaderOptions would be filtered | `(inputValue: string, path: CoCascaderOption[]): boolean` | - |
| `sorter` | Optional | `(a: CoCascaderOption[], b: CoCascaderOption[], inputValue: string): number` | - |
