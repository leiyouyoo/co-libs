...
import COS from 'cos-js-sdk-v5'; // 发送图片、文件等消息需要的 COS SDK
...
export function parseText(payload, emojiMap, emojiUrl: string) {
  const renderDom: Array<{ name: string; text?: string; src?: string }> = [];
  // 文本消息
 ...
 */
export async function getUserProfile(userIDList: Array<any>) {
  await onSDKReady(() => {});
...
export function onMessage(fun: () => void) {
  // function(fun) {
...
export function onKickedOut(fun: () => void) {
  // 收到被踢下线通知
