...
export function createTextMessage(userID: string, type: string, payload: string) {
  ...
  const textMessage = tim.createTextMessage({
    to: userID,
...
export function createImageMessage(userID: string, type: string, params: any, fun?: (e) => void) {
  ...
  const imgMessage = tim.createImageMessage({
    to: userID,
    conversationType: type,
    payload: {
      file: params,
...
export function createFileMessage(userID: string, type: string, params: any, fun?: (e) => void) {
  ...
  const fileMessage = tim.createFileMessage({
    to: userID,
    conversationType: type,
    payload: {
      file: params,
...
export async function getGroupProfile(groupID: string) {
  ...
  await onSDKReady(() => {});
  return tim.getGroupProfile(params);
}
...
export async function updateGroupProfile(groupInfo: GroupInfoUpdate) {
  await onSDKReady(() => {});
  return tim.updateGroupProfile(groupInfo);
}
/**
 *  获取个人资料
...
export async function updateMyProfile(myInfo: MyInfoCheck) {
  await onSDKReady(() => {});
  return tim.updateMyProfile(myInfo);
}
/**
 * 获取群成员列表
 * @param groupID 群组Id
...
export function getGroupMemberlist(groupID: string, count: number = 100, offset: number = 0) {
  const promise = tim.getGroupMemberList({ groupID, count, offset });
  return promise;
}
/**
 *  获取其他用户资料
...
export async function getMessageList(conversationID: string, nextReqMessageID?: string) {
  await onSDKReady(() => {});
  if (!nextReqMessageID) {
    return tim.getMessageList({ conversationID, count: 15 });
  }
  if (nextReqMessageID) {
    return tim.getMessageList({ conversationID, nextReqMessageID, count: 15 });
  }
...
export function onSDKReady(fun: () => void) {
  // 监听事件，例如：
  // 收到离线消息和会话列表同步完毕通知，接入侧可以调用 sendMessage 等需要鉴权的接口
  // event.name - TIM.EVENT.SDK_READY
  subject.subscribe(() => {
...
export function onMessage(fun: (e) => void) {
  // function(fun) {
  // 收到推送的单聊、群聊、群提示、群系统通知的新消息，可通过遍历 event.data 获取消息列表数据并渲染到页面
  // event.name - TIM.EVENT.MESSAGE_RECEIVED
  // event.data - 存储 Message 对象的数组 - [Message]
  // });
  tim.on(TIM.EVENT.MESSAGE_RECEIVED, function message(e) {
    fun(e);
  });
}

export function onEventError(fun: () => void) {
...
export function onSDKNotReady(fun: () => void) {
  // 收到 SDK 进入 not ready 状态通知，此时 SDK 无法正常工作
  // event.name - TIM.EVENT.SDK_NOT_READY
  tim.on(TIM.EVENT.SDK_NOT_READY, fun);
}

export async function onKickedOut(fun: () => void) {
  // 收到被踢下线通知
