...
export class Walker extends RuleWalker {
  ...
  visitIdentifier(identifier: ts.Identifier) {
    ...
    // For namespace imports that are referring to`@co, we store the
    ...
    // For export declarations that are referring to`@co, the identifier
    // can be immediately updated to the new name.
    if (isExportSpecifierNode(identifier) && isDelonExportDeclaration(identifier)) {
      return this._createFailureWithReplacement(identifier);
    }

    // For import declarations that are referring to`@co, the name of
