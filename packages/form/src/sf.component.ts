...
export function useFactory(schemaValidatorFactory: SchemaValidatorFactory, cogSrv: CoConfigService) {
...
@Component({
  ...
  providers: [
    WidgetFactory,
    {
      ...
      useFactory,
      deps: [SchemaValidatorFactory, CoConfigService],
    },
    TerminatorService,
  ],
  host: {
...
export class SFComponent implements OnInit, OnChanges, OnDestroy {
  ...
  /**
   * 根据路径获取表单元素属性
   * @param path [路径](https://www.cityocean.com/form/qa#path)
   */
