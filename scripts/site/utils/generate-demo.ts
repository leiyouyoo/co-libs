...
import { getCode, genUpperName, genUrl, generateDoc, genSelector, genComponentName } from './utils';
...
function fixExample(item: any, filePath: string, config: ModuleConfig) {
  item.componentIndexName = genComponentName(`${genUpperName(`${config.name}-${item.name}-index`)}Component`);
  // item.componentIndexName = `${item.componentIndexName.split('\\').map((key: any) => genUpperName(key)).join('')}`;
...
export function generateDemo(
  ...
  demos
    ...
    .forEach((markdownData: any, index: number) => {
      ...
      item.id = genSelector(item.componentName);
      item.componentName = genComponentName(item.componentName);
